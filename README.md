...
- **`enablePm2LoginAutostart(name)` / `disablePm2LoginAutostart(name)`** — auto-start at login/boot. **Windows:** `pm2 save` + a hidden `HKCU\…\Run` entry running `pm2 resurrect` (no admin; works where Startup-folder writes and `schtasks /create` are blocked). **POSIX:** pm2's native `pm2 save` + `pm2 startup` / `pm2 unstartup` (may print a one-off sudo command).
